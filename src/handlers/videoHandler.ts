...
>(
  async function handleVideoRequestImpl(
    ...
    const { startTimedOperation, endTimedOperation, setCurrentContext } =
    ...
    try {
      ...
      const { getCachedResponse, cacheResponse } =
        await import('../services/cacheManagementService');
          ...
          if (kvCacheEnabled) {
            ...
            if (hasIMQueryParams) {
              if (videoOptions.derivative) {
                vhLogger.debug(                  'Using derivative-based caching for IMQuery request',
                  {
                    imwidth,
                    imheight,
                    hasIMRef,
                ...
                vhLogger.debug(                  'IMQuery request without mapped derivative',
                  {
                    imwidth,
                    imheight,
                    hasIMRef,
            ...
            if (
              envWithCtx.executionCtx &&
              typeof envWithCtx.executionCtx.waitUntil === 'function'
            ) {
              envWithCtx.executionCtx.waitUntil(
                storeInKVCache(
                  ...
                  sourcePath,
                  responseClone,
                  videoOptionsWithIMQuery as unknown as TransformOptions
                )
                  .then((success) => {
                    if (success) {
                      ...
                      vhLogger.debug('Failed to store in KV cache', {
                        path: sourcePath,
                      });
                    }
                  })
      ...
      if (
        ...
        (videoOptions.format && videoOptions.format.toLowerCase() === 'm4a')
      ) {
      ...
      const errorResponse = new Response(
        `Error processing video: ${err instanceof Error ? err.message : 'Unknown error'}`,
        {
          status: 500,
          headers: {
