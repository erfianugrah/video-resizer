...
>(
  async function handleRequestWithCachingImpl(
    ...
    const videoOptions = {
      ...
      autoplay: url.searchParams.has('autoplay')
        ? url.searchParams.get('autoplay') === 'true'
        : undefined,
      muted: url.searchParams.has('muted') ? url.searchParams.get('muted') === 'true' : undefined,
      duration: url.searchParams.has('duration')
        ? url.searchParams.get('duration') || ''
        : undefined,
      fps: url.searchParams.has('fps')
        ? parseInt(url.searchParams.get('fps') || '', 10)
        : undefined,
    };
    ...
    try {
      ...
      cacheLogger.info('Video transformation request with caching', {
        path: url.pathname,
        requestId: requestContext.requestId,
      ...
      logErrorWithContext(
        'Failed to initialize request context',
        err,
        {
          url: request.url,
          path: url.pathname,
