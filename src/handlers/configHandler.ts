    ...
    try {
      ...
      addBreadcrumb(context, 'ConfigHandler', 'Processing configuration upload request', {
        ...
        headers: Object.fromEntries(
          [...request.headers].map(([k, v]) => [k, k.toLowerCase() === 'authorization' ? '***' : v])
        ),
      });
      ...
      if (!configKV) {
        ...
        logErrorWithContext(
          `No ${bindingName} KV namespace binding found`,
          new Error('KV namespace not found'),
          {
      ...
      try {
        ...
        if (!env.CONFIG_API_TOKEN) {
          logErrorWithContext(
            'CONFIG_API_TOKEN not set in environment',
        ...
        logErrorWithContext(
          'Authorization header validation failed',
          error,
          {
            error: errMessage,
          },
      ...
      try {
        ...
        if (success) {
          ...
          logErrorWithContext(
            'Failed to store configuration in KV',
            new Error('KV storage operation failed'),
            {
              duration: `${duration}ms`,
            },
            'ConfigHandler'
        ...
        logErrorWithContext(
          'Exception during configuration storage',
          error,
          {
            error: errMessage,
            stack: errStack,
      ...
      logErrorWithContext(
        'Unhandled error in configuration upload handler',
        error,
        {
          error: errMessage,
          stack: errStack,
      ...
      try {
        ...
        logErrorWithContext(
          'Failed to add breadcrumb for unhandled error',
          breadcrumbError,
          {
            error:
              breadcrumbError instanceof Error ? breadcrumbError.message : String(breadcrumbError),
          },
          'ConfigHandler'
        );
      }
  ...
  async function handleConfigGetImpl(request: Request, env: Env): Promise<Response> {
    try {
      ...
      try {
        ...
        logErrorWithContext(
          'Exception during configuration retrieval',
          error,
          {
            error: errMessage,
            stack: errStack,
      ...
      logErrorWithContext(
        'Unhandled error in configuration retrieval handler',
        error,
        {
          error: errMessage,
          stack: errStack,
