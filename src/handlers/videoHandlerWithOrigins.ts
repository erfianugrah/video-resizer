...
const vhLogger = createCategoryLogger('VideoHandlerWithOrigins');
    ...
    try {
        ...
        if (kvCacheEnabled) {
          ...
          try {
            ...
            kvResponse = await getFromKVCache(
              env,
              sourcePath,
              initialVideoOptions as unknown as TransformOptions,
              request
            );
          } catch (err) {
      ...
      if (kvResponse) {
        ...
        if (speculativeOriginPromise) {
          vhLogger.debug('Cancelling speculative origin resolution due to cache hit', {
            path: url.pathname,
          });
        }
      ...
      if (env && videoOptions && !skipCache) {
        ...
        if (kvCacheEnabled && ctx) {
          ...
          ctx.waitUntil(
            (async () => {
              try {
                vhLogger.debug('Storing response in KV (background)', {
                  path: sourcePath,
                  hasIMQuery: hasIMQueryParams,
                  derivative: videoOptions.derivative,
                });

                const storedInKV = await storeInKVCache(
                  env,
                  sourcePath,
                  responseForKV,
                ...
                if (storedInKV) {
                  vhLogger.debug('Successfully stored in KV (background)', {
                    path: sourcePath,
                  });
                } else {
                  vhLogger.debug('KV storage failed (background)', {
                    path: sourcePath,
                  });
                }
              } catch (err) {
                vhLogger.error('Error storing in KV (background)', {
                  error: err instanceof Error ? err.message : String(err),
                  path: sourcePath,
                });
              }
      ...
      if (
        ...
        (videoOptions.format && (videoOptions.format as string).toLowerCase() === 'm4a')
      ) {
      ...
      logErrorWithContext(
        'Error handling video request with Origins',
        err,
        {
          url: request.url,
...
async function transformVideoWithOrigins(
  ...
  try {
    ...
    // Import getCurrentContext to get the request context for ResponseBuilder
