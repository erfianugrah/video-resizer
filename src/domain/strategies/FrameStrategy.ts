...
const logger = createCategoryLogger('FrameStrategy');
...
export class FrameStrategy implements TransformationStrategy {
  ...
  prepareTransformParams(context: TransformationContext): TransformParams {
    ...
    logger.debug('Prepared frame transformation params', params);
  ...
  async validateOptions(options: VideoTransformOptions): Promise<void> {
    ...
    if (options.format) {
      ...
      if (!['jpg', 'png'].includes(options.format)) {
        throw ValidationError.invalidFormat(options.format, ['jpg', 'png'], context);
      }
