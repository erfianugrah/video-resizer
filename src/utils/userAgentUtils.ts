...
const logger = createCategoryLogger('UserAgentUtils');
...
export function getVideoSizeForDeviceType(
  ...
  const standardQualities = configQualities.length
    ? configQualities
    : [360, 480, 720, 1080, 1440, 2160];
  const sortedQualities = [
    ...(availableQualities.length ? availableQualities : standardQualities),
  ].sort((a, b) => a - b);
...
export function detectBrowserVideoCapabilities(userAgent = ''): {
  ...
  logger.debug('Detecting browser capabilities from User-Agent', {
  ...
  try {
    ...
    if (browserCapabilities) {
      ...
      if (browserCapabilities.hevc) {
        ...
        const hasMatch = patterns.some((pattern) => {
          try {
            ...
            logger.error('Invalid regex pattern for HEVC', { pattern });
            return false;
          }
        });
        ...
        const hasExclusion =
          exclusions?.some((exclusion) => {
            try {
              ...
              logger.error('Invalid regex exclusion for HEVC', { exclusion });
              return false;
            }
          }) || false;
      ...
      if (browserCapabilities.av1) {
        ...
        const hasMatch = patterns.some((pattern) => {
          try {
            ...
            logger.error('Invalid regex pattern for AV1', { pattern });
            return false;
          }
        });
        ...
        const hasExclusion =
          exclusions?.some((exclusion) => {
            try {
              ...
              logger.error('Invalid regex exclusion for AV1', { exclusion });
              return false;
            }
          }) || false;
      ...
      if (browserCapabilities.vp9) {
        ...
        const hasMatch = patterns.some((pattern) => {
          try {
            ...
            logger.error('Invalid regex pattern for VP9', { pattern });
            return false;
          }
        });
        ...
        const hasExclusion =
          exclusions?.some((exclusion) => {
            try {
              ...
              logger.error('Invalid regex exclusion for VP9', { exclusion });
              return false;
            }
          }) || false;
      ...
      if (browserCapabilities.webm) {
        ...
        const hasMatch = patterns.some((pattern) => {
          try {
            ...
            logger.error('Invalid regex pattern for WebM', { pattern });
            return false;
          }
        });
        ...
        const hasExclusion =
          exclusions?.some((exclusion) => {
            try {
              ...
              logger.error('Invalid regex exclusion for WebM', { exclusion });
              return false;
            }
          }) || false;
      ...
      // Check for WebM support
      if (/Chrome|Firefox|Edge|Opera/.test(userAgent)) {
        capabilities.supportsWebM = true;
      }
    }
  } catch (error) {
    logger.error('Error getting browser capabilities from configuration', {
  ...
  logger.debug('Browser capabilities detection result', { capabilities });
  return capabilities;
}
