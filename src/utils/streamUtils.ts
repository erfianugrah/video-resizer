  ...
  try {
    ...
    if (requestContext) {
      addBreadcrumb(
        requestContext,
        'RangeRequest',
        `Processing range request: bytes=${start}-${end}/${totalSize}`,
        {
    ...
    streamLogger.debug('Processing range request', {
    ...
    const streamProcessing = async () => {
      ...
      try {
              ...
              for (let i = 0; i < totalSegments; i++) {
                ...
                try {
                  ...
                  if (writer.desiredSize === null || writer.desiredSize < 0) {
                    ...
                    streamLogger.debug('Writer is already closed or being closed', {
                      segment: i,
                      totalSegments,
                      start,
                      end,
                  ...
                  try {
                    ...
                    await withTimeout(writer.write(segment), timeoutMs, 'Segment write timed out');
                  } catch (writeError) {
                    // Check if this is a "readable side" error
                    const errorMessage =
                      writeError instanceof Error ? writeError.message : String(writeError);
                    if (
                      errorMessage.includes('readable side') ||
                      errorMessage.includes('TransformStream')
                    ) {
                      streamLogger.debug('Stream was closed by client during write', {
                        segment: i,
                        totalSegments,
                        error: errorMessage,
                      });
                  ...
                  if (writer.desiredSize === null || writer.desiredSize < 0) {
                    streamLogger.debug('Writer was closed during write operation', {
                      segment: i,
                      totalSegments,
                      start,
                      end,
                  ...
                  streamLogger.error('Error writing stream segment', {
        ...
        streamLogger.debug('Range request streaming completed', {
          bytesRead,
          bytesWritten,
        ...
        if (streamError instanceof DOMException && streamError.name === 'AbortError') {
          streamLogger.debug('Stream processing aborted by client', {
            bytesRead,
            bytesWritten,
          });
        } else {
          streamLogger.error('Error processing stream for range request', {
    ...
    if (requestContext?.executionContext?.waitUntil) {
      requestContext.executionContext.waitUntil(
        streamProcessing().catch((err) => {
          streamLogger.error('Background stream processing error', {
            error: err instanceof Error ? err.message : String(err),
          ...
          if (
            ...
            try {
              ...
              // Ignore abort errors
              streamLogger.debug('Failed to abort writer after error', {
...
export async function handleRangeRequest(
  ...
  try {
    ...
    if (!rangeHeader || !rangeHeader.startsWith('bytes=')) {
      streamLogger.debug('No valid range header found', {
    ...
    if (!contentLength || contentLength <= 0) {
      streamLogger.debug('No valid content length for range processing', {
    ...
    if (!parsedRange) {
      ...
      streamLogger.debug('Unsatisfiable range request', {
        range: rangeHeader,
    ...
    streamLogger.debug('Valid range request found', {
      rangeHeader,
    ...
    logErrorWithContext('Error handling range request', error, { rangeHeader }, 'StreamUtils');
