...
export async function loadFromKV(
  ...
  if (!kvNamespace) {
    logger.error('KV namespace not available', {
  ...
  try {
    ...
    if (!result) {
      logger.debug('No configuration found in KV', {
        key: CONFIG_KEY,
      });
    ...
    logger.error('Error fetching configuration from KV', {
      error: error instanceof Error ? error.message : String(error),
      key: CONFIG_KEY,
    });
    metrics.kvErrorCount = (metrics.kvErrorCount as number) + 1;
 ...
 */
async function getFromKV(kvNamespace: KVNamespace, key: string): Promise<unknown> {
  try {
