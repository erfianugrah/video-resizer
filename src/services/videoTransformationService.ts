...
const vtsLogger = createCategoryLogger('VideoTransformationService');
...
/**
 * Log a debug message using the category logger
 */
function logDebug(message: string, data?: Record<string, unknown>): void {
  vtsLogger.debug(message, data);
}

/**
 * Log a warning message using the category logger
 */
function logWarn(message: string, data?: Record<string, unknown>): void {
  vtsLogger.warn(message, data);
}
...
>(
  async function transformVideoImpl(
    ...
    try {
      ...
      logDebug('Video transformation successful', {
        url: request.url,
