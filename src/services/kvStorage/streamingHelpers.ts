    ...
    for (let i = 0; i < chunksToFetch.length; i++) {
      ...
      try {
        ...
        if (bytesSentForRange === 0) {
          ...
          throw new Error(
            `Failed to fetch initial chunk: ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`
          );
        }
      ...
      if (chunkArrayBuffer.byteLength !== chunkInfo.size) {
        ...
        if (isAcceptableDifference) {
          logDebug(
            '[GET_VIDEO] Minor chunk size difference detected, continuing with actual size',
            {
              chunkKey: chunkInfo.key,
              expectedSize: chunkInfo.size,
              actualSize: chunkArrayBuffer.byteLength,
          ...
          if (clientRange.start <= chunkEndInVideo && clientRange.end >= chunkStartInVideo) {
            ...
            chunkInfo.sliceEnd = Math.min(
              chunkArrayBuffer.byteLength,
              clientRange.end - chunkStartInVideo + 1
            );
          }
        } else {
      ...
      try {
        ...
        if (bytesSentForRange % 1000000 === 0) {
          // Log every ~1MB
          logDebug('[GET_VIDEO] Range request streaming progress', {
            bytesSentForRange,
            percentComplete: Math.round(
              (bytesSentForRange / (clientRange.end - clientRange.start + 1)) * 100
            ),
          });
        }
      } catch (writeError) {
    ...
    if (!isStreamAborted && !isStreamClosed) {
      try {
        ...
        logDebug('[GET_VIDEO] Aborting stream due to error', {
          ...
          isTransformStreamError:
            error instanceof Error &&
            (error.message.includes('TransformStream') || error.message.includes('readable side')),
        });
