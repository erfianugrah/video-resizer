...
import { createBaseMetadata, handleVersionStorage, logStorageSuccess, storeWithRetry } from './storageHelpers';
...
async function storeTransformedVideoImpl(
  ...
  if (totalActualVideoBytes <= MAX_VIDEO_SIZE_FOR_SINGLE_KV_ENTRY) {
    ...
    const cacheTags = generateCacheTags(sourcePath, options, new Headers({
