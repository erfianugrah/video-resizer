    ...
    try {
      ...
      if (attemptCount === 1) {
        logDebug('[STORE_HELPER] Storage size diagnostics', {
          ...
          cacheTagsSize: metadata.cacheTags
            ? new Blob([JSON.stringify(metadata.cacheTags)]).size
            : 0,
        });
            ...
            new Error('Metadata too large'),
            {
              ...
              fieldsSize: {
                ...
                customData: metadata.customData
                  ? new Blob([JSON.stringify(metadata.customData)]).size
                  : 0,
              },
            },
            'KVStorageService.store'
...
export async function handleVersionStorage(
  ...
  if (!env?.VIDEO_CACHE_KEY_VERSIONS) {
    logDebug(
      '[STORE_VIDEO] No VIDEO_CACHE_KEY_VERSIONS namespace available, skipping version storage',
      {
        key,
        cacheVersion,
        hasEnv: !!env,
  ...
  try {
    ...
    if (env && 'executionCtx' in env && (env as any).executionCtx?.waitUntil) {
        ...
        (async () => {
          ...
          while (attemptCount < maxRetries && !success) {
            try {
              ...
              // Log the retry attempt
              logDebug(
                '[STORE_VIDEO] KV rate limit hit during version storage, retrying with backoff',
                {
                  key,
                  attempt: attemptCount,
                  maxRetries,
      ...
      // Check that env exists before using it
      ...
      while (attemptCount < maxRetries && !success) {
        try {
          ...
          // Log the retry attempt
          logDebug(
            '[STORE_VIDEO] KV rate limit hit during version storage, retrying with backoff (direct)',
            {
              key,
              attempt: attemptCount,
              maxRetries,
