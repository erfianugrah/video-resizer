...
import { StreamingChunkProcessor } from './streamChunkProcessor';
...
async function processStreamInChunks(
  ...
  try {
      ...
      const uploadPromise = uploadQueue.add(async () => {
        ...
        try {
          ...
          if (buffer instanceof SharedArrayBuffer) {
            ...
            const sharedSlice = buffer.slice(
          ...
          if (dataToStore.byteLength !== chunkSize) {
            throw new Error(
              `Chunk buffer size mismatch during copy: expected ${chunkSize}, got ${dataToStore.byteLength}`
            );
          }
    ...
    if (chunkKeys.length > 0) {
      ...
      if (!manifestSuccess) {
        logDebug('[STREAM_STORE] Failed to store manifest', { key });
        return {
          success: false,
          totalSize: totalProcessedBytes,
          chunkKeys,
    ...
    logDebug('[STREAM_STORE] Error processing stream chunks', {
      key,
