...
async function fetchVideoImpl(
  ...
  logger.debug(`[${requestId}] Starting video fetch operation`, {
    path,
    hasRequest: !!request,
    url: request?.url,
  ...
  logger.debug('Video fetch request analysis', {
    path,
  ...
  if (isMediaTransformationSubrequest) {
    logger.debug('Detected media-transformation subrequest', { path });

    // First, determine if R2 should be used based on storage priority
    const shouldUseR2 =
      config.storage?.priority?.includes('r2') &&
    ...
    if (shouldUseR2) {
      logger.debug('Using R2 for media-transformation subrequest', { path });
      ...
      logger.debug('Video key for subrequest', {
        originalPath: path,
      ...
      try {
        ...
        if (result) {
          logger.debug('Found video in R2 bucket for subrequest', { r2Key });
        ...
        if (r2Key !== normalizedPath) {
          logger.debug('Video not found with transformed key, trying normalized path', {
            r2Key,
            normalizedPath,
          });
          ...
          if (fallbackResult) {
            logger.debug('Found video in R2 bucket using normalized path', {
              normalizedPath,
            });
      ...
      logger.debug('R2 not available for media-transformation subrequest', {
  ...
  logger.debug('Detailed storage configuration', {
  ...
  for (const storageType of availableStorage) {
    ...
    if (storageType === 'r2' && config.storage?.r2?.enabled === true && env.VIDEOS_BUCKET) {
      ...
      if (result) {
        logger.debug('R2 fetch successful', {
          size: result.size,
          contentType: result.contentType,
        });
    ...
    if (!result && storageType === 'remote' && config.storage?.remoteUrl) {
      ...
      logger.debug('Trying remote URL with detailed diagnostics', {
        path,
      ...
      if (result) {
        logger.debug('Remote fetch successful', {
          size: result.size,
          contentType: result.contentType,
        });
    ...
    if (!result && storageType === 'fallback' && config.storage?.fallbackUrl) {
      ...
      logger.debug('Trying fallback URL with detailed diagnostics', {
        path,
      ...
      if (result) {
        logger.debug('Fallback fetch successful', {
          size: result.size,
          contentType: result.contentType,
        });
    ...
    if (result) {
      ...
      logger.debug(`[${requestId}] Successfully found video in storage`, {
