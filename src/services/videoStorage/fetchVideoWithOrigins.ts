...
async function fetchVideoWithOriginsImpl(
  ...
  logger.debug(`[${requestId}] Starting video fetch operation with Origins`, {
    path,
    hasRequest: !!request,
    url: request?.url,
  ...
  if (!config.origins || getOriginsCount(config.origins) === 0) {
    logger.debug('No Origins configuration found, falling back to legacy method', {
      path,
  ...
  logger.debug('Video fetch request analysis with Origins', {
    path,
  ...
  if (isMediaTransformationSubrequest) {
    logger.debug('Detected media-transformation subrequest with Origins', {
      path,
    });
    ...
    if (r2Resolution && env.VIDEOS_BUCKET) {
      logger.debug('Using R2 for media-transformation subrequest', {
        path,
      ...
      try {
        ...
        if (result) {
          logger.debug('Found video in R2 bucket for subrequest', {
          ...
          if (requestContext) {
            addBreadcrumb(
              requestContext,
              'Storage',
              'R2 fetch successful for subrequest (Origins)',
              {
                key: r2Resolution.resolvedPath,
  ...
  if (matchingOrigins.length === 0) {
    logger.debug('No matching origin found for path', { path });
  ...
  logger.debug('Found matching origins for path', {
    path,
  ...
  for (const { origin, match: originMatch } of matchingOrigins) {
    ...
    if (options?.excludeSources && options.excludeSources.length > 0) {
      ...
      logger.debug('Applied source exclusions', {
    ...
    if (sources.length === 0) {
      logger.debug('All sources excluded for origin, trying next', {
        originName: origin.name,
      });
      continue;
    }

    logger.debug('Trying origin for path', {
      path,
      originName: origin.name,
    ...
    for (const source of sources) {
      ...
      logger.debug(`Trying ${source.type} source`, {
        originalPath: path,
      ...
      if (source.type === 'r2' && env.VIDEOS_BUCKET) {
        ...
        logger.debug('Attempting R2 fetch', {
          r2Url,
        ...
        if (result) {
          logger.debug('R2 fetch successful (Origins)', {
            size: result.size,
            contentType: result.contentType,
          });
      ...
      // Try remote URL
        ...
        logger.debug('Attempting remote fetch', {
          remoteUrl,
          baseUrl: source.url,
          path: resolvedPath,
        });
        ...
        if (source.auth?.enabled) {
          logger.debug('Remote source requires authentication', {
            sourceType: source.type,
        ...
        if (result) {
          logger.debug('Remote fetch successful (Origins)', {
            size: result.size,
            contentType: result.contentType,
      ...
      // Try fallback URL
        ...
        logger.debug('Attempting fallback fetch', {
          fallbackUrl,
          baseUrl: source.url,
          path: resolvedPath,
        });
        ...
        if (source.auth?.enabled) {
          logger.debug('Fallback source requires authentication', {
            sourceType: source.type,
            authType: source.auth.type,
            authEnabled: source.auth.enabled,
          });
        ...
        if (result) {
          logger.debug('Fallback fetch successful (Origins)', {
            size: result.size,
            contentType: result.contentType,
            hasAuth: !!source.auth?.enabled,
          });
      ...
      if (result) {
        ...
        logger.debug(`[${requestId}] Successfully found video with Origins`, {
    ...
    logger.debug('Video not found in origin, trying next', {
      originName: origin.name,
