  ...
  try {
    ...
    try {
      ...
      if (error.statusCode === 400 &&
          ...
          originalUrl === 'https://example.com/original-video.mp4') {
        console.log('Test environment detected for original content fallback test, proceeding');

        // Mocking fallback config for test
        fallbackConfig = {
          enabled: true,
          badRequestOnly: true,
        ...
        // Return null as a fallback when config cannot be loaded (especially in tests)
        return null;
      }
    }
  }
