...
export default Sentry.withSentry<EnvVariables>(
  ...
  }),
  {
    async fetch(request: Request, env: EnvVariables, ctx: ExecutionContext): Promise<Response> {
      // Create request context at the entry point, passing execution context for waitUntil
      ...
      try {
        ...
        if (requestUrl.pathname === '/internal/container-result' && request.method === 'POST') {
          ...
          const kvKey = requestUrl.searchParams.get('kvKey');
          ...
          try {
            ...
            // Use the pre-computed KV key from the callback URL.
            // This key was generated by the same generateKVKey function
            // used on the read path, guaranteeing an exact match.
            const stored = await storeTransformedVideoWithStreaming(
              cacheKV,
              kvPath,
              storeResponse,
              { version, compression, env: env as any } as any,
              undefined, // ttl
              kvKey || undefined // pre-computed key
            );
