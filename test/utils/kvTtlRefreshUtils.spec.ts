...
describe('kvTtlRefreshUtils', () => {
  ...
  describe('refreshKeyTtl', () => {
    ...
    it('should refresh TTL when conditions are met', async () => {
      ...
      expect(mockNamespace.put).toHaveBeenCalledWith('test-key', '', {
        metadata: {
          ...mockMetadata,
  ...
  describe('checkAndRefreshTtl', () => {
    it('should skip refresh if metadata is missing', async () => {
      const result = await checkAndRefreshTtl(mockNamespace, 'test-key', null as any);
    ...
    it('should skip refresh if createdAt is missing', async () => {
      const result = await checkAndRefreshTtl(mockNamespace, 'test-key', { something: 'else' });
    ...
    it('should use default TTL if expiresAt is missing', async () => {
      ...
      const result = await checkAndRefreshTtl(mockNamespace, 'test-key', metadataWithoutExpiry);
