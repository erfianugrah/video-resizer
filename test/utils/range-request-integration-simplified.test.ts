...
describe('Stream Utils Integration', () => {
  ...
  it('should handle fallback content correctly', async () => {
    ...
    const result = await streamUtils.handleRangeRequest(
      ...
      'bytes=0-499',
      {
        bypassCacheAPI: true,
        preserveHeaders: true,
        handlerTag: 'Fallback-Handler',
        fallbackApplied: true
      }
    );

    // Verify the response is correct for fallback
    expect(result.status).toBe(206);
    expect(result.headers.get('Content-Range')).toBe('bytes 0-499/1000');
    expect(result.headers.get('Content-Length')).toBe('500');
    expect(result.headers.get('X-Range-Handled-By')).toBe('Fallback-Handler');
