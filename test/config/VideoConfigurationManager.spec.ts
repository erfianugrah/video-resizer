...
import {
  ...
  PathPatternSchema,
  VideoConfigSchema,
} from '../../src/config/VideoConfigurationManager';
import { z } from 'zod';
...
describe('VideoConfigurationManager', () => {
  ...
  describe('Initialization and Singleton Pattern', () => {
    ...
    it('should validate the configuration on initialization', () => {
      ...
      expect(() => VideoConfigurationManager.getInstance(invalidConfig)).toThrow(
        ConfigurationError
      );
    });
  });
  ...
  describe('Configuration Access Methods', () => {
    ...
    it('should throw an error for non-existent derivative', () => {
      ...
      expect(() => manager.getDerivative('nonexistent')).toThrow(ConfigurationError);
    });
  ...
  describe('Configuration Modification', () => {
    ...
    it('should throw an error when adding an invalid path pattern', () => {
      ...
      expect(() =>
        manager.addPathPattern(invalidPattern as unknown as z.infer<typeof PathPatternSchema>)
    ...
    it('should throw an error when updating with invalid configuration', () => {
      ...
      expect(() =>
        manager.updateConfig(invalidUpdate as unknown as Partial<z.infer<typeof VideoConfigSchema>>)
      ).toThrow(ConfigurationError);
    });
      ...
      const testOrigins = [
        {
          ...
          captureGroups: ['videoId'],
          sources: [
            { type: 'r2' as const, priority: 1, bucketBinding: 'VIDEOS_BUCKET', path: '$1' },
