...
class MockKVNamespace implements KVNamespace {
  ...
  async put(key: string, value: string | ReadableStream | ArrayBuffer | FormData, options?: any): Promise<void> {
  ...
  async getWithMetadata(key: string, type: 'text'): Promise<{ value: string | null; metadata: any }>;
  async getWithMetadata(key: string, type: 'json'): Promise<{ value: any; metadata: any }>;
  async getWithMetadata(key: string, type: 'arrayBuffer'): Promise<{ value: ArrayBuffer | null; metadata: any }>;
  async getWithMetadata(key: string, type: 'stream'): Promise<{ value: ReadableStream | null; metadata: any }>;
  ...
  async list(options?: any): Promise<{ keys: { name: string; expiration?: number; metadata?: any }[] }> {
...
describe('PresignedUrlCacheService', () => {
  ...
  describe('storePresignedUrl and getPresignedUrl', () => {
    it('should store and retrieve a presigned URL', async () => {
      ...
      const presignedUrl = 'https://test-bucket.s3.amazonaws.com/videos/test.mp4?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=...';
      ...
      // Store the URL
      const storeResult = await storePresignedUrl(mockKV, path, presignedUrl, originalUrl, options);
      ...
      const cachedEntry = await getPresignedUrl(mockKV, path, retrieveOptions);
    ...
    it('should handle URL expiration', async () => {
      ...
      await storePresignedUrl(mockKV, path, presignedUrl, originalUrl, {
        storageType: 'remote',
      ...
      let cachedEntry = await getPresignedUrl(mockKV, path, {
        storageType: 'remote',
        authType: 'aws-s3-presigned-url',
        env: mockEnv
      });
      ...
      cachedEntry = await getPresignedUrl(mockKV, path, {
        storageType: 'remote',
        authType: 'aws-s3-presigned-url',
        env: mockEnv
      });
  ...
  describe('refreshPresignedUrl', () => {
    it('should refresh an expiring URL', async () => {
      ...
      // Refresh the URL
      const refreshResult = await refreshPresignedUrl(mockKV, entry, {
      ...
      const cachedEntry = await getPresignedUrl(mockKV, path, {
        storageType: 'remote',
        authType: 'aws-s3-presigned-url',
        region: 'us-east-1',
        service: 's3',
