...
// Set up module mocks (hoisted to top)
vi.mock('../../src/config', () => ({
  VideoConfigurationManager: {
    getInstance: vi.fn().mockReturnValue({
      ...
      getPathPatterns: vi.fn().mockReturnValue([]),
    }),
  },
...
beforeEach(async () => {
  ...
  // Re-setup config mock return values after clearing
  const { VideoConfigurationManager } = await import('../../src/config');
  (VideoConfigurationManager.getInstance as any).mockReturnValue({
    getCachingConfig: vi.fn().mockReturnValue({
      fallback: {
        enabled: true,
        badRequestOnly: true,
...
describe('Error Handler Service', () => {
  ...
  describe('fetchOriginalContentFallback', () => {
    it('should fetch original content when a 400 error occurs and fallback is enabled', async () => {
      ...
      expect(fallbackResponse?.headers.get('X-Original-Error-Type')).toBe(
    ...
    it('should not fetch original content for non-400 errors when badRequestOnly is true', async () => {
      ...
      const mockError = new VideoTransformError(
