...
vi.mock('aws4fetch', () => {
  return {
    AwsClient: class MockAwsClient {
      ...
      sign(request: Request, options?: { aws?: { signQuery?: boolean }; signQuery?: boolean; expiresIn?: number }) {
        ...
        if (options?.aws?.signQuery || options?.signQuery) {
          ...
          url.searchParams.append('X-Amz-Credential', 'AKIAIOSFODNN7EXAMPLE/20220505/us-east-1/s3/aws4_request');
          ...
          url.searchParams.append('X-Amz-Signature', '1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef');
        ...
        headers.set('authorization', 'AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/20220505/us-east-1/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature=1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef');
...
describe('videoStorageService', () => {
  ...
  describe('AWS S3 Header Authentication', () => {
    it('should add AWS signature headers when configured for header auth', async () => {
      ...
      // Call the service
      await fetchVideo('test-video.mp4', config, mockEnv);
