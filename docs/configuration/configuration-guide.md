...
#### 4. Audio Mode (`mode=audio`)

- Extracts or processes the audio track from a video
- Useful for generating audio-only versions of video content
- Example: `https://cdn.example.com/videos/sample.mp4?mode=audio`
...
| `mode`        | string  | 'video'   | Transformation mode: 'video', 'frame', 'spritesheet', or 'audio' |
